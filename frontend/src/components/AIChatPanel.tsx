...
export function AIChatPanel({ isOpen, onClose, paginaId, paginaTitulo, onApplyContent }: AIChatPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 pointer-events-none">
      ...
      {/* Panel */}
      <div
        className="absolute top-0 right-0 bottom-0 w-[420px] max-w-[90vw] bg-[#1e1e1e] border-l border-[#2e2e2e] shadow-2xl flex flex-col pointer-events-auto animate-slide-in"
        style={{ animation: "slideIn 0.2s ease-out" }}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#2e2e2e] shrink-0">
          <div className="flex items-center gap-2 min-w-0">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-[#a8dcff] to-[#8b5cf6] flex items-center justify-center">
              <Sparkles size={14} className="text-white" />
            </div>
            <div className="min-w-0">
              <h3 className="text-sm font-semibold text-[#ffffff]">Assistente IA</h3>
              {paginaTitulo && (
